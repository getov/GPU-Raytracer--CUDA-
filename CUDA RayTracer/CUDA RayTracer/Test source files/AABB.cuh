...
struct Triangle {
	...
	/*__device__
	Triangle(const char* a, const char* b, const char* c);*/
...
struct BBox {
	...
	{
		vmin.x = dev_min(vmin.x, vec.x); vmax.x = dev_max(vmax.x, vec.x);
		vmin.y = dev_min(vmin.y, vec.y); vmax.y = dev_max(vmax.y, vec.y);
		vmin.z = dev_min(vmin.z, vec.z); vmax.z = dev_max(vmax.z, vec.z);
	}
	...
	{
		return (vmin.x - 1e-3 <= v.x && v.x <= vmax.x + 1e-3 &&
		        vmin.y - 1e-3 <= v.y && v.y <= vmax.y + 1e-3 &&
		        vmin.z - 1e-3 <= v.z && v.z <= vmax.z + 1e-3);
	}
